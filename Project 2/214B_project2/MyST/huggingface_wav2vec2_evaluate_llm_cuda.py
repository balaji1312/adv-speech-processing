...
model = Wav2Vec2ForCTC.from_pretrained(checkpoint_model).cuda()
...
if model_name=='gpt2':
    ...
    model_dir = '/data/balaji/Project2_Group1/214B_project2/langauge_model/libri_full'
    llm_model = BertForMaskedLM.from_pretrained(model_dir)
    llm_model.eval()
    # Load pre-trained model tokenizer (vocabulary)
    llm_tokenizer = BertTokenizer.from_pretrained('bert-large-uncased')
elif model_name=='roberta':
    llm_model = RobertaForMaskedLM.from_pretrained('roberta-base').cuda()
    llm_model.eval()
    # Load pre-trained model tokenizer (vocabulary)
    llm_tokenizer = RobertaTokenizer.from_pretrained("roberta-base")
elif model_name=='llm':
    llm_model = AutoModelForCausalLM.from_pretrained("databricks/dolly-v2-3b")
    llm_model.eval()
    # Load pre-trained model tokenizer (vocabulary)
    llm_tokenizer = AutoTokenizer.from_pretrained("databricks/dolly-v2-3b")
else:
    base_dir = '/data/balaji/Project2_Group1/214B_project2/langauge_model/gpt2/'
    llm_tokenizer = GPT2Tokenizer.from_pretrained('gpt2')
    llm_model = GPT2LMHeadModel.from_pretrained(base_dir + model_name).cuda()
    llm_model.eval()
...
def score_sentences(sentence):
    ...
    if model_name=='bert' or model_name=='bert_ft':
        ...
        with torch.inference_mode():
            loss = -1*llm_model(masked_input, labels=labels).loss
    
    elif model_name=='roberta':
        tensor_input = llm_tokenizer.encode(sentence, return_tensors='pt')
        tensor_input = tensor_input.to(device='cuda')
        repeat_input = tensor_input.repeat(tensor_input.size(-1)-2, 1)
        mask = torch.ones(tensor_input.size(-1) - 1).diag(1).to(device='cuda')[:-2]
        masked_input = repeat_input.masked_fill(mask == 1, llm_tokenizer.mask_token_id)
        labels = repeat_input.masked_fill( masked_input != llm_tokenizer.mask_token_id, -100)
        with torch.inference_mode():
            loss = -1*llm_model(masked_input, labels=labels).loss.cpu()
        ...
        loss=-1*llm_model(tensor_input, labels=tensor_input)[0].cpu()
...
def map_to_result(batch):
    with torch.no_grad():
        input_values = torch.tensor(batch["input_values"], device="cpu").unsqueeze(0).to('cuda')
